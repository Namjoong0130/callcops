...
- **발신 측**: 통화 음성에 **사람이 인지할 수 없는** 128비트 인증 정보를 **실시간(Causal)**으로 삽입
...
| 실시간성 (Real-time) | **Causal 설계 (미래 데이터 참조 불가)** | Latency < 200ms |
...
| 최종 출력 | [B, 1, 8000] | [B, 1, 8000] | 원본 + 섭동 = 워터마크 오디오 |
 ...
 핵심은 **Stride=1**과 **Causal Padding**을 유지하여 시간 해상도(T)가 처음부터 끝까지 동일하며, **미래의 샘플을 참조하지 않는다**는 점이다. 모든 변환은 채널(C) 차원에서만 일어나며, 시간 축의 각 샘플이 1:1로 보존된다. 이 덕분에 40ms 프레임 경계가 정확하게 유지되며 실시간 스트리밍이 가능하다.
...
#### 3.2.4 실시간 스트리밍 전략: Mini-Batch & History Context

단일 프레임(40ms) 단위로 인코더를 호출하면 연산 오버헤드가 크고 컨텍스트 부족으로 음질이 저하될 수 있다. CallCops는 이를 해결하기 위해 **Mini-batch 스트리밍** 기술을 사용한다.

- **Mini-Batch (32 frames)**: 약 1.28초 분량의 오디오를 묶어서 한 번에 처리한다. 이는 ONNX 호출 횟수를 8배 줄여주며 연산 효율을 극대화한다.
- **History Context (8 frames)**: 현재 배치 처리 시, 직전 배치의 마지막 320ms(8프레임) 오디오를 입력의 앞에 붙여서 컨텍스트를 제공한다.
  - 입력 텐서: `[History(8) + Current(32)] = 40 frames`
  - 출력 텐서: 모델의 Causal 특성 덕분에 앞선 8프레임의 출력은 버리고 뒤의 32프레임만 취해도 파형이 완벽하게 이어진다.
- **Zero Look-ahead**: 모든 Convolution 레이어는 `padding=(kernel_size-1, 0)`의 형태인 Causal Padding을 사용하여, 현재 샘플 $t$를 생성할 때 $t+1$ 이후의 데이터는 절대 참조하지 않는다.

이러한 설계를 통해 **스트리밍 중에도 전체 오디오를 한 번에 처리한 것과 물리적으로 동일한 결과**를 얻으면서 낮은 지연 시간을 유지한다.

**정리하면**, 인코더의 출력은 원본 오디오와 **동일한 형태 `[B, 1, T]`** 이다.
 각 샘플에 $[-0.25, +0.25]$ 범위의 미세한 값이 더해진 것이 전부이므로, 사람의 귀에는 차이가 거의 들리지 않는다.

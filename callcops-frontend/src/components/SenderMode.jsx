...
export default function SenderMode({ onBack }) {
    ...
    const handleStopRecording = useCallback(async () => {
        ...
        try {
            ...
            // Generate message (128 bits: 16 sync + 32 timestamp + 64 auth + 16 CRC)
            const now = Math.floor(Date.now() / 1000);
            const syncPattern = 0xAAAA; // 16 bits
            const timestamp = now & 0xFFFFFFFF; // 32 bits
            const authCode = BigInt(Math.floor(Math.random() * 0xFFFFFFFFFFFFFFFF)); // 64 bits
            ...
            // Calculate CRC-16 on first 112 bits
            ...
            // Visualize message as bit probs (for display)
        ...
        return (
            <div className="grid grid-cols-16 gap-0.5">
                {bitProbs.map((prob, i) => (
                    <div
                        key={i}
                        className={`w-3 h-3 rounded-sm ${prob > 0.5 ? 'bg-green-500' : 'bg-red-500'}`}
                    />
                ))}
            </div>
        );
    ...
    if (state === 'idle') {
        return (
            <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 flex flex-col items-center justify-between py-12 px-6">
                <button
                    onClick={onBack}
                    className="self-start text-gray-400 hover:text-white flex items-center gap-2"
                >
                ...
                <button
                    ...
                    className="w-20 h-20 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center shadow-lg shadow-green-500/30 hover:scale-110 transition-transform active:scale-95"
                >
                ...
                <div className="flex-1 flex flex-col items-center justify-center">
                    ...
                    <div className="flex items-center gap-1 h-16">
                        {Array(30).fill(0).map((_, i) => {
                            const height = 8 + Math.sin(i * 0.3 + Date.now() / 100) * 20 + Math.random() * 8;
                            return (
                                <div
                                    key={i}
            ...
            <div className="min-h-screen bg-gradient-to-b from-green-900/30 via-gray-900 to-gray-900 flex flex-col items-center justify-between py-12 px-6">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <p className="text-gray-400 mb-8 text-center">
                        워터마크가 삽입된 오디오 파일이 준비되었습니다
                    </p>

                    {/* Bit Matrix Preview */}
                    <div className="bg-gray-800/30 rounded-xl p-3 border border-gray-700/50 mb-8">
                        {renderBitMatrix()}
                    </div>
                    ...
                    <button
                        ...
                        className="flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl text-white font-medium hover:opacity-90 transition-opacity"
                    >

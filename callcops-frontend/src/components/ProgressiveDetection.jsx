...
export function ProgressiveDetection({ 
  ...
  // Get bit color
  const getBitColor = (prob, revealed) => {
    if (!revealed) return 'bg-gray-700';
    const hue = prob * 120;
    const sat = 70 + Math.abs(prob - 0.5) * 60;
    const light = 40 + Math.abs(prob - 0.5) * 20;
    ...
    <div className="glass rounded-xl p-4 space-y-4">
        ...
        {Array.from({ length: 128 }).map((_, i) => {
          ...
          return (
            <div
              ...
              style={{ 
                backgroundColor: isRevealed ? getBitColor(prob, true) : '#374151'
              }}
            >
      ...
      <div className="flex justify-center gap-4 text-[10px] text-gray-500">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-sm" style={{ backgroundColor: 'hsl(120, 100%, 50%)' }} />
          1 (확신)
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-sm" style={{ backgroundColor: 'hsl(60, 70%, 50%)' }} />
          불확실
        </span>
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-sm" style={{ backgroundColor: 'hsl(0, 100%, 50%)' }} />
          0 (확신)
        </span>
      </div>

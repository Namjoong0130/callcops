...
export function CRCVerificationPanel({ decodedMessage, originalMessage = null }) {
  const verification = useMemo(() => {
    ...
    return {
      ...
      dataHex: messageToHex(decodedMessage.slice(0, 120))
    };
  }, [decodedMessage]);
    ...
    <div className="glass rounded-xl p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-surface/50 rounded-lg p-3">
          <p className="text-[10px] text-gray-500 mb-1">Expected CRC-8</p>
          <p className="font-mono text-sm text-gray-300">
            0x{verification.expectedCRC.toString(16).padStart(2, '0').toUpperCase()}
          </p>
        </div>
        <div className="bg-surface/50 rounded-lg p-3">
          <p className="text-[10px] text-gray-500 mb-1">Actual CRC-8</p>
          <p className={`font-mono text-sm ${verification.isValid ? 'text-green-400' : 'text-red-400'}`}>
            0x{verification.actualCRC.toString(16).padStart(2, '0').toUpperCase()}
          </p>
        </div>
      </div>
